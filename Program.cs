...
app.MapGet("/api/customer/{platform}/{status}", [Authorize] async (MetaContext db, string platform, int status) => await db.MailCrmFinal.Where(a => a.IsSpam == 0 && a.Platform.Equals(platform) && a.Status == (status == 0 ? false : true)).ToListAsync());
