...
app.MapGet("/api/customer/status/{platform},{status}", [Authorize] async (MetaContext db, string platform, int status) => await db.MailCrmFinal.Where(a => a.IsSpam == 0 && a.Platform == platform && a.Status == (status == 0 ? false : true)).ToListAsync());
